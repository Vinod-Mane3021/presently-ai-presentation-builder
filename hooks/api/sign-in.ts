...
export const signInUser = async (json: RequestType): Promise<ResponseType> => {
  try {
    const endpoint = `${Env.NEXT_PUBLIC_API_BASE_URL}/auth/sign-in`;
...
export const signInWithOAuthProvider = async (
  json: SignInWithOauthProviderType
): Promise<ResponseType> => {
  try {
    const endpoint = `${Env.NEXT_PUBLIC_API_BASE_URL}/auth/sign-in/oauth`;
    const response = await fetch(endpoint, {
      method: "POST",
      body: JSON.stringify(json),
    });
